...
void web_loop()
{
    ...
    // Re-announce mDNS every 5 seconds
    static _millis_t lastMDNSannounce = upTime;
#define MDNS_ANNOUNCE_TIMEOUT (5 * 1000)
    if (upTime - lastMDNSannounce > MDNS_ANNOUNCE_TIMEOUT)
    {
        ...
        if (wifi_netif)
        {
            esp_err_t err = mdns_netif_action(wifi_netif, MDNS_EVENT_ANNOUNCE_IP4);
            if (err != ESP_OK)
            {
                ESP_LOGE(TAG, "mdns_netif_action failed: %s", esp_err_to_name(err));
            }
        }
#endif
    }
    ...
    static time_t lastClockSet = 0;
    if (clockSet != lastClockSet)
    {
        lastClockSet = clockSet;
        add_dynamic_mdns();
    }
    else
    {
