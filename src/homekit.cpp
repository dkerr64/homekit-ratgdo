...
void homekit_task_entry(void *ctx)
{
    ...
    config.name = device_name_rfc952;
    config.manufacturer = (char *)"ratCloud llc";
    config.model = (char *)"ratgdo32";
    config.serial_num = serial_number;
    config.fw_rev = (char *)AUTO_VERSION;
    config.hw_rev = NULL;
    ...
    hap_serv_add_char(gdo_svc, hap_char_name_create((char *)"ratgdo"));
    ...
    hap_serv_add_char(gdo_svc, hap_char_active_create(false));
    ...
    ESP_LOGI(TAG, "Set esp-homekit-sdk log level to warn/error only");
    ...
    while (true)
    {
        ...
        if (xQueueReceive(gdo_notif_event_q, &e, portMAX_DELAY))
        {
            ...
            case HomeKitNotifDest::Motion:
                ...
                value.b = e.value.b;
                break;
            case HomeKitNotifDest::Active:
                dest = hap_serv_get_char_by_uuid(gdo_svc, HAP_CHAR_UUID_STATUS_ACTIVE);
                value.b = e.value.b;
                break;
            }
            if (dest)
...
static int gdo_svc_set(hap_write_data_t write_data[], int count, void *serv_priv, void *write_priv)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        {
            ESP_LOGI(TAG, "set door state: %d", (int)write->val.u);
            switch (write->val.u)
            {
            case TGT_OPEN:
                open_door();
            ...
            case TGT_CLOSED:
                ...
                *(write->status) = HAP_STATUS_SUCCESS;
                break;
            default:
                ESP_LOGE(TAG, "invalid target door state set requested: %d", (int)write->val.u);
...
{
    GDOEvent e;
    e.dest = HomeKitNotifDest::Active;
    e.value.b = garage_door.active;
    if (!gdo_notif_event_q || xQueueSend(gdo_notif_event_q, &e, 0) == errQUEUE_FULL)
    {
        ESP_LOGE(TAG, "could not queue homekit notif of active state %d", garage_door.active);
    }
}
